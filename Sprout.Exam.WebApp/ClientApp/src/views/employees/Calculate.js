...
export class EmployeeCalculate extends Component {
                    ...
                    <div className="form-row">
                        ...
                        {this.state.typeId === 1 ?
                            <div className='form-group col-md-6'>
                                ...
                                <input type='text' pattern="[0-9]*" className='form-control' id='inputAbsentDays4' onChange={this.handleChange.bind(this)} value={this.state.absentDays} name="absentDays" placeholder='Absent Days' />
                            </div> :
                            <div className='form-group col-md-6'>
                                <label htmlFor='inputWorkDays4'>Worked Days: </label>
                                <input type='text' pattern="[0-9]*" className='form-control' id='inputWorkDays4' onChange={this.handleChange.bind(this)} value={this.state.workedDays} name="workedDays" placeholder='Worked Days' />
                            </div>
                        }
                    </div>
    ...
    async calculateSalary() {
        ...
        if (this.state.typeId === 1 && this.state.absentDays >= 22) {
            alert("Employee reached maximum absent days")
        }
        else if (this.state.typeId === 2 && this.state.workedDays === 0) {
            alert("Employee need to work at least one day")
        }
        else {
            const response = await fetch('api/employees/' + this.state.id + '/calculate', requestOptions);
            const data = await response.json();
            this.setState({ loadingCalculate: false, netIncome: Number(data.toFixed(2)).toLocaleString('en', { minimumFractionDigits: 2 }) });
        }
    ...
    async getEmployee(id) {
        ...
        if (response.status === 200) {
            const data = await response.json();
            this.setState({ id: data.id, fullName: data.fullName, birthdate: data.formattedBirthdate, tin: data.tin, typeId: data.employeeTypeId, loading: false, loadingCalculate: false });
        }
        else {
